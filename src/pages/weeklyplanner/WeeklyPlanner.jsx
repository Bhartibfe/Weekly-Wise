...
const DAY_THEMES = {
  ...
  sunday: {
    bg: "bg-gradient-to-br from-indigo-200 to-purple-200",
    emoji: "🌞",
    color: "text-purple-800",
  },
};
...
const getInitialState = () => {
  return {
    ...
    selectedDay: localStorage.getItem("planner-selected-day")
      ? new Date(localStorage.getItem("planner-selected-day"))
      : null,
    timeRange: JSON.parse(
      localStorage.getItem("planner-time-range") || '{"start":8,"end":20}'
    ),
  };
};
...
const WeeklyPlanner = () => {
  ...
  const [{ events, tasks, view, selectedDay, timeRange }, setState] =
  ...
  useEffect(() => {
    ...
    if (selectedDay)
      saveToStorage("planner-selected-day", selectedDay.toISOString());
      ...
      const centerModal = () => {
        if (modalRef.current) {
          ...
          modalRef.current.style.top = `${
            scrollTop + (viewportHeight - modalHeight) / 2
          }px`;
        }
      };
  ...
  const taskActions = {
    ...
    convertToEvent: (day, task) => {
      ...
      const now = new Date();
      const hour =
        now.getHours() >= timeRange.start && now.getHours() < timeRange.end
          ? now.getHours()
          : 12;
  ...
  const TimeRangeSelector = () => (
    <div className="mb-6 flex items-center space-x-4 bg-#03030A">
      <label className="font-medium text-gray-700">Time Range:</label>
      <select
        value={timeRange.start}
        onChange={(e) =>
          updateState({
            timeRange: { ...timeRange, start: parseInt(e.target.value) },
          })
        }
      ...
      <select
        ...
        onChange={(e) =>
          updateState({
            timeRange: { ...timeRange, end: parseInt(e.target.value) },
          })
        }
        className="border border-purple-200 rounded-md p-2 focus:ring-2 focus:ring-purple-300 focus:outline-none"
      >
  ...
  const TaskItem = ({ day, task }) => (
    <div className="flex items-center mb-2 group">
      ...
      <input
        ...
        className={`flex-grow px-2 py-1 rounded-md focus:outline-none focus:bg-purple-50 transition-colors ${
  ...
  const EmptyTaskLines = ({ day, count }) => (
    <>
      {Array.from({ length: count }).map((_, idx) => (
        <div
          key={idx}
          className="flex items-center mb-2 h-8 border-b border-dotted border-gray-200"
        >
  ...
  const DayCard = ({ day }) => {
    ...
    return (
      <div
        className={`bg-white/80 backdrop-blur-sm rounded-xl border border-purple-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:scale-105 ${
          isToday ? "ring-4 ring-purple-300" : ""
        }`}
      >
        {/* Day Header */}
        <div
  ...
  return (
    <div className="p-8 bg-#03030A  shadow-sm">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-4">
        {/* Header with icon and title */}
        <div className="flex items-center justify-center mb-4">
          <CalendarDays className="w-10 h-10 mr-3 text-purple-600" />
            ...
            <div className="bg-white/80 p-1 rounded-full shadow-sm border border-purple-100 flex">
              <button
                ...
                onClick={() =>
                  updateState({
                    view: "day",
                    selectedDay: selectedDay || currentDate,
                  })
                }
              >
                Day
      ...
      {view === "day" && selectedDay && (
        <div className="rounded-xl shadow-lg bg-white/80 backdrop-blur-sm border border-purple-100 overflow-hidden">
          ...
          <div
            className={`${
              getDayTheme(selectedDay).bg
            } p-4 flex justify-between items-center`}
          >
            <div className="flex items-center">
              <span className="text-3xl mr-3">
                {getDayTheme(selectedDay).emoji}
              </span>
            ...
            <div className="flex space-x-2">
              <button
                onClick={() =>
                  updateState({ selectedDay: addDays(selectedDay, -1) })
                }
          ...
          <div
            className="relative"
            style={{ height: `${(timeRange.end - timeRange.start) * 60}px` }}
          >
            {/* Time markers */}
            {Array.from({ length: timeRange.end - timeRange.start + 1 }).map(
              (_, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`absolute left-0 right-0 border-t ${
                      isEvenHour ? "border-purple-100" : "border-gray-100"
                    }`}
                    style={{ top: `${idx * 60}px` }}
                  >
                    <div
            ...
            <div className="absolute left-16 right-4 top-0 bottom-0">
              {eventActions
                .getForDay(selectedDay)
                .filter(
                  (event) =>
                    !(
                      event.end.getHours() < timeRange.start ||
                      event.start.getHours() >= timeRange.end
                    )
                )
                .map((event, idx) => (
                  <div
              ...
              {Array.from({
                length: (timeRange.end - timeRange.start) * 4,
              }).map((_, idx) => {
      ...
      <EventModal
        ...
        setRemoveOriginalTask={(value) =>
          updateModalState({ removeOriginalTask: value })
        }
      />
