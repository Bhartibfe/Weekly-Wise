...
const LinksPage = () => {
    ...
    try {
      ...
      if (savedLinks) {
        ...
        const uniqueCategories = [
          "All",
          ...new Set(parsedLinks.map((link) => link.category).filter(Boolean)),
        ];
  ...
  const handleAddLink = () => {
    ...
    // Force save to localStorage (might help if there's an async issue)
    setTimeout(() => {
      localStorage.setItem("links", JSON.stringify(links));
      console.log("Forced save after add:", links);
    }, 100);
  };
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto">
                ...
                <button
                  ...
                  className={`p-2 ${
                    viewMode === "grid"
                      ...
                      : "bg-white text-gray-500 hover:bg-purple-50 hover:text-purple-500"
                  } transition-colors`}
        ...
        {filteredLinks.length === 0 && (
          <div className="flex flex-col items-center justify-center p-12 bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-purple-100">
            ...
            <h3 className="text-xl font-medium mb-2 bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-pink-500">
              No links found
            </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {filteredLinks.map((link) => {
              const originalIndex = links.findIndex(
                (b) => b.url === link.url && b.title === link.title
              );
