...
const NotesPage = () => {
  ...
  useEffect(() => {
    ...
    const openNoteId = Object.entries(tagDropdownStates).find(
      ([, isOpen]) => isOpen
    )?.[0];
  ...
  const positionTagDropdown = (noteId) => {
    ...
    setTimeout(() => {
      if (dropdownRefs.current[noteId]) {
        ...
        const dropdownEl =
          dropdownRefs.current[noteId].querySelector("div.absolute");
  ...
  const createNewTag = (e, noteId) => {
    ...
    if (!noteId) {
      noteId = Object.entries(tagDropdownStates).find(
        ([, isOpen]) => isOpen
      )?.[0];
    }
  ...
  const getFilteredNotes = () => {
    ...
    if (activeView === "important") {
      filtered = filtered.filter(
        (note) => Array.isArray(note.tags) && note.tags.includes("Important")
      );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 ml-1">
            Capture your ideas, organize with tags, and access them anywhere.
          </p>
        </div>
                    ...
                    <div className="flex flex-wrap gap-1 items-center">
                      {Array.isArray(note.tags) &&
                        note.tags.map((tag) => (
                          <div
                            key={`${note.id}-${tag}`}
                            className={`px-2 py-1 mb-1 text-xs rounded-full ${getTagColor(
                              tag
                            )} flex items-center transition-all hover:shadow-sm`}
                          >
                            <span>{tag}</span>
                            <button
                            ...
                            <div className="max-h-64 overflow-y-auto">
                              {availableTags
                                ...
                                .map((tag) => (
                                  ...
                                  >
                                    <div className="flex-shrink-0 mr-2 text-purple-400">
                                      ∷
                                    </div>
                                    <div
                                      className={`px-2 py-0.5 rounded-full text-sm ${tag.color}`}
                                    >
                                      {tag.name}
                                    </div>
                                  </div>
                  ...
                  <td colSpan="5" className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-500">
                      ...
                      selectedGroupTags.length > 0 ? (
                        ...
                        <p className="text-lg">
                          Select at least one tag to view notes
                        </p>
                      ) : searchTerm.trim() !== "" ? (
